...
module Auths
  class EmailVerificationService
    ...
    def verify_email(token:)
      return { success: false, message: I18n.t('errors.messages.blank') } if token.blank?
      ...
      if email_confirmation_token.nil? || email_confirmation_token.expires_at < Time.current
        { success: false, message: I18n.t('errors.messages.invalid') }
      else
        user = User.find_by(id: email_confirmation_token.user_id)
        if user.nil?
          { success: false, message: I18n.t('errors.messages.unverified_email') }
        elsif user.email_confirmed
          { success: false, message: I18n.t('errors.messages.already_confirmed') }
        else
          ...
          if user.save
            ...
            { success: true, user_id: user.id, message: I18n.t('confirmations.email_verified') }
          else
